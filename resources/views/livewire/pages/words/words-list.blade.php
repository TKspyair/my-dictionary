 ...
 * 画面全体のマージン、パディングの調整
 * フォントや色などの調整
 * ドロップダウンメニューの大きさ縮小　※CSSでカスタム必要かも
 */
...
<article>
    ...
    <header class="d-flex justify-content-between align-items-center">
        ...
        <nav class="d-flex justify-content-end align-items-center">
            ...
            <section class="dropdown pe-2">
                <!-- ソートマーク -->
                <button type="button" class="me-2 border-0 bg-transparent" data-bs-toggle="dropdown"> {{-- dropdown-toggleは不要なアイコンが表示されてしまうため使用しない --}}
                ...
                * dropdawn-menuにスタイルを当てることで大きさを調整している(min-widthを上書きしないと調整不可)
                ...
                <ul class="dropdown-menu" style="min-width: 50px !important;">
                    ...
                    <li class="dropdown-item p-1" wire:click="sortBy('created_at')">
                        ...
                        <span @class([
                            'mx-2 fs-6 py-1 px-2',
                            'text-white bg-primary border-0 rounded' => $this->sortColumn === 'created_at',
                            ])>登録順
                        </span>
                    ...
                    <li class="dropdown-item p-1"
                        wire:click="sortBy('word_name')">
                        {{-- NOTE: 選択すると文字色と背景色が変化し,選択したことを視覚的にわかりやすくする --}}
                        <span @class([
                            'mx-2 fs-6 py-1 px-2',
                            'text-white bg-primary border-0 rounded' => $this->sortColumn === 'word_name',
                            ])>名前順
                        </span>
            ...
            <section>
                ...
                <button type="button" class="border-0 bg-transparent" 
                    data-bs-toggle="offcanvas" data-bs-target="#words.words-list-offcanvas">
                    <i class="bi bi-funnel fs-4"></i>
                </button>
                
                <!-- オフキャンバス -->
                <div class="offcanvas offcanvas-bottom rounded-top-4" style="height: 80vh !important; "tabindex="-1" id="words.words-list-offcanvas"> {{-- オフキャンバスを開く data-bs-toggle="offcanvas" data-bs-target="#[指定のid] --}}
                    <!-- ヘッダー部 -->
                    ...
                    <div class="offcanvas-body">
                        <form wire:submit="applyFilter">
                            ...
                            <div class="d-flex flex-column mt-4">
                                ...
                                <div class="mt-2">
                                    <!-- ～の日時 -->
                                    <input type="date" class="fs-6" wire:model="filterBeginDate"> <!-- wire:modelが値を管理するため、value属性は不要 -->
                                ...
                                * flex-wrapでタグを左詰めで自動改行するように表示する
                                ...
                                <div class="d-flex flex-wrap mt-2">
                                    @foreach ($this->tags as $tag)
                                        <div class="me-3" x-data="{ isSelected: false }" wire:key="{{ $tag->id }}">
                                            {{-- NOTE:  タグ選択の仕組み
                                            ...
                                            * input要素にbtn-checkを適用し、チェックボックスを非表示にlabel要素をボタンとして扱う
                                            ...
                                            --}}
                                            <input type="checkbox" name="selectedTagIds[]" value="{{ $tag->id }}" id="{{ $tag->id }}"
                                            ...
                                            * x-on:clickで押すたびに状態を反転させる式を使用することで、切り替えを実現している
                                            --}}
                                            <label for="{{ $tag->id }}" class="btn border-0 rounded fs-6 py-1 px-2" 
                            ...
                            <div class="position-fixed start-50 translate-middle-x" style="bottom: 50px;">
                                <button type="submit" class="text-white border-0 rounded-3 bg-primary py-1 px-3">
                                    <span class="fs-6">
                                        絞り込み
                                    </span>
                                </button>
    ...
    <section class="mt-1">
        ...
        * ページネーション部はもともとsection閉じタグの下に配置していたが、要素の大きさの問題で期待通りの場所に配置できなかったため、section要素内に配置している
        ...
        <div class="position-fixed start-50 translate-middle-x" style="bottom: 100px;">
